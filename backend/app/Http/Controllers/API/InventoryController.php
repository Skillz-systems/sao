...
class InventoryController extends Controller
{
  ...
  public function createSubitem(Request $request)
  {
    ...
    $subitem->userid = $id;
    if(!empty($request->input("hasb3"))){
      $subitem->hasb3 = $request->input("hasb3");
    }else{
      $subitem->hasb3 = "no";
    }
    $subitem->save();
  ...
  public function viewavailablestocks(Request $request)
  {
    $query = $request->all();
    if(array_key_exists('perpage', $query))
    {//check if perpage is in query string
    ...
    {//check if page is in query string
      ...
      $subitems = Stockaddition::with('subitem', 'item',  'Addedby')->orderBy('id', 'desc')->where('status',Stockaddition::$available)->paginate($perpage)->toarray();
      $data = $subitems["data"];
      $page = $subitems["current_page"];
      $totalpages = ceil($subitems["total"]/$perpage);
  ...
  { 
    ...
    if(!empty($query["status"]) and $query["status"] != "all"){
      ...
      if($query["status"] == "available"){
        $statusCode = Stockaddition::$available;
      }
